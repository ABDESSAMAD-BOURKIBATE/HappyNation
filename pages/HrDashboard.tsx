...
import { PieChart, Settings, LogOut, AlertTriangle, Trash2, Plus, Users, UserPlus, Upload, User as UserIcon, Edit2, MessageSquare, X, CheckCircle } from 'lucide-react';
...
interface HrDashboardProps {
    ...
    onUpdateEmployee: (emp: EmployeeWithAuth) => void;
...
const HR_STATS = [
    ...
    { name: 'Healthy', value: 65, color: 'bg-green-500' },
    { name: 'Moderate Stress', value: 23, color: 'bg-yellow-500' },
];
...
export const HrDashboard = ({ isDarkMode, questions, onAddQuestion, onDeleteQuestion, employees, onAddEmployee, onUpdateEmployee, onDeleteEmployee, onSendFeedback }: HrDashboardProps) => {
        ...
        <div className="flex min-h-screen overflow-hidden">
            ...
            <div className={`w-24 lg:w-72 border-r p-6 flex flex-col hidden md:flex backdrop-blur-xl ${isDarkMode ? 'bg-slate-900/40 border-slate-800' : 'bg-white/50 border-slate-200'} `}>
                <div className="flex items-center gap-4 mb-12 px-2">
                    <img src="/logo.png" className="w-10 h-10 object-contain drop-shadow-lg" alt="Logo" />
                    <span className="font-bold text-xl tracking-tight hidden lg:block">Happynation</span>
                </div>
                ...
                <nav className="space-y-4 flex-1">
                    ...
                    <NavItem active={hrTab === 'SETTINGS'} onClick={() => setHrTab('SETTINGS')} isDark={isDarkMode} icon={<Settings size={24} />}><span className="hidden lg:block">Settings</span></NavItem>
                </nav>
                ...
                <header className="flex justify-between items-center mb-10">
                    <div>
                        ...
                        <p className={`${textMuted} mt-2`}>
                            {hrTab === 'OVERVIEW' ? 'Organization Health Overview' : (hrTab === 'EMPLOYEES' ? 'Manage Workforce' : 'System Configuration')}
                        </p>
                    </div>
                            ...
                            <Card isDark={isDarkMode} className="lg:col-span-2 !p-0 overflow-hidden">
                                ...
                                <div className="h-[350px] p-6">
                                    <ResponsiveContainer width="100%" height="100%">
                                        <BarChart data={TEAM_DATA} barGap={8}>
                            ...
                            <Card isDark={isDarkMode} className="!p-0 overflow-hidden flex flex-col">
                                ...
                                <div className="p-6 space-y-4 flex-1 overflow-y-auto">
                                    {notifications.length > 0 ? notifications.map((notif, i) => {
                                        ...
                                        return (
                                            <div key={i} className={`flex items-start gap-4 p-4 rounded-3xl border transition-colors cursor-pointer group ${isDarkMode ? 'bg-slate-800/30 border-slate-700/30 hover:bg-slate-800' : 'bg-slate-50 border-slate-100 hover:bg-slate-100'} `}>
                                    ...
                                    <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6 p-8">
                                        {employees.map((emp, idx) => (
                                            <div key={idx} className={`relative flex flex-col gap-4 p-6 rounded-3xl border transition-all hover:shadow-lg ${isDarkMode ? 'bg-slate-800/50 border-slate-700 hover:border-slate-600' : 'bg-white border-slate-200 hover:border-blue-200'} `}>
                                                ...
                                                <button onClick={() => handleFeedbackClick(emp.id)} className={`w-full flex items-center justify-center gap-2 py-2 rounded-xl text-xs font-bold transition-colors ${isDarkMode ? 'bg-blue-500/10 text-blue-400 hover:bg-blue-500 hover:text-white' : 'bg-blue-50 text-blue-600 hover:bg-blue-100'} `}>
                                    ...
                                    <div className="grid grid-cols-2 gap-4">
                                        ...
                                        <div className="space-y-2 col-span-2">
                                            ...
                                            <input
                                                ...
                                                className={`w-full p-4 rounded-xl border bg-transparent outline-none font-bold transition-all focus:ring-2 focus:ring-blue-500/20 ${isDarkMode ? 'border-slate-700' : 'border-slate-200'} ${isEditingEmployee ? 'opacity-50 cursor-not-allowed' : ''}`}
